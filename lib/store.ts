...
/**
 * Per-session chip derived from history. Same heuristic as the singular-session
 * store from Task 0 — used on rehydrate to keep the chip honest after a reload.
 *
 ...
 * care about *the active session* should use the `useActiveSession()` helper
 * instead. Persisted under `lucid:sessions:v2`.
 */
export const useSessions = create<SessionsStoreShape>()(
  persist(
    (set, get) => ({
      ...
      setState: (sessionId, state) =>
        set((s) => {
          ...
          return {
            sessions: patchSession(s.sessions, sessionId, { state }),
          };
        }),
      ...
      finishVoiceTurn: (sessionId, transcript, duration) => {
        ...
        const nextName = isFirstTurn && session.name === session.greekId
          ? deriveName(text)
          : session.name;
        ...
        // whisper-style transcribe pause, then agent
        setTimeout(() => {
      ...
      approveLatestPlan: (sessionId) => {
        ...
        for (let i = session.turns.length - 1; i >= 0; i--) {
          ...
          if (t.kind === 'agent' && t.plan.length > 0 && !t.approved) {
            ...
            // Release the server-side approval gate. Best-effort — if the
    ...
    }),
    {
      ...
      // Persist the durable map + order + active pointer. Drop ghost turns and
      // reset transient mic fields per session.
      partialize: (state) => ({
        order: state.order,
...
/**
 * Returns all sessions in creation order.
 *
 * `useShallow` is critical here: the selector builds a fresh array on every
 * call, and with Zustand v5's default `Object.is` equality React would treat
 * each call as a new snapshot, infinite-loop the render guard.
 */
export const useSessionList = (): Session[] =>
  useSessions(useShallow((s) => s.order.map((id) => s.sessions[id]).filter(Boolean)));

/** Count sessions that are not IDLE — drives the `N LIVE` chip. */
